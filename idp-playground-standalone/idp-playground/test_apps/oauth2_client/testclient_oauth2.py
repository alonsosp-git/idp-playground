# -*- coding: utf-8 -*-
"""
IDP-Playground -- OAuth 2.0 Authorization Code Demo
File : testclient_oauth2.py   Port: 5002

Authorization Code Flow with PKCE:
  1. Client generates code_verifier + code_challenge (S256)
  2. Redirect to /oauth2/authorize with code_challenge
  3. IDP-Playground redirects back with ?code=...
  4. Client POSTs code + code_verifier to /oauth2/token
  5. Receives access_token + id_token

Run: python testclient_oauth2.py
"""

import os, sys, io, json, secrets, base64, hashlib, datetime
...
PUBLIC_URL      = os.environ.get("OAUTH2_PUBLIC_URL", "http://localhost:5002")
REDIRECT_URI    = PUBLIC_URL + "/oauth2/callback"
...
app = Flask(__name__)
app.secret_key = "idp-playground-oauth2-client-fixed-session-key-2024"
...
app.config["SESSION_COOKIE_NAME"] = "va_oauth2_session"
...
@app.route("/version")
def _version():
    return {"build": BUILD_VERSION, "protocol": "OAuth2"}
...
def _show_error(e):
    ...
    html = ("<html><body style='background:#0b0f1a;color:#e6edf3;font-family:monospace;padding:30px'>"
        "<h1 style='color:#f87171'>OAuth2 client error (" + str(code) + ")</h1>"
...
def _register():
    ...
    try:
        ...
        found = next((a for a in apps if a["name"] == "OAuth2 Demo Client"), None)
        if found:
            _client_id = found["client_id"]
            rot = requests.post(
                f"{IDPPLAYGROUND_BASE}/api/fs/applications/{found['id']}/rotate-secret",
                timeout=4).json()
            _client_secret = rot.get("client_secret","")
            return True
        reg = requests.post(f"{IDPPLAYGROUND_BASE}/api/fs/applications", json={
            "name": "OAuth2 Demo Client", "protocol": "OAuth",
            "redirect_uris": REDIRECT_URI,
            "allowed_scopes": SCOPES,
            "icon_emoji": "⚡", "brand_color": "#34d399",
        }, timeout=4).json()
        _client_id     = reg["client_id"]
        _client_secret = reg["client_secret"]
        print(f"  [IDP-Playground] OAuth2 app registered  cid={_client_id[:14]}...", flush=True)
        return True
...
:root{--bg:#05080d;--bg2:#090e16;--bg3:#0e1620;--b:1px solid #172333;--b2:1px solid #1f3349;
  --p:#34d399;--green:#34d399;--red:#f87171;--v:#e8ff47;--text:#d4e4f5;--muted:#4a6685;}
...
.hex{width:36px;height:36px;background:var(--p);
  ...
  box-shadow:0 0 18px rgba(52,211,153,.4);}
...
.badge-p{background:rgba(52,211,153,.1);border:1px solid rgba(52,211,153,.25);color:var(--p);}
.card{background:var(--bg2);border:1px solid rgba(52,211,153,.15);border-radius:11px;overflow:hidden;margin-bottom:14px;}
...
.btn-p{background:var(--p);color:#000;box-shadow:0 0 14px rgba(52,211,153,.3);}
.btn-p:hover{box-shadow:0 0 26px rgba(52,211,153,.5);transform:translateY(-1px);}
...
.tbox{background:var(--bg);border:var(--b);border-radius:7px;padding:11px 13px;
  ...
  word-break:break-all;overflow-wrap:anywhere;max-width:100%;line-height:1.7;cursor:pointer;}
.tbox:hover{border-color:var(--p);}
...
.gtag{display:inline-flex;padding:3px 9px;border-radius:6px;font-size:11px;font-weight:700;
  background:rgba(52,211,153,.1);color:var(--green);border:1px solid rgba(52,211,153,.2);margin:2px;}
...
.av{flex-shrink:0;}
.av{width:34px;height:34px;border-radius:8px;background:rgba(52,211,153,.2);color:var(--green);
...
.chip-p{background:rgba(52,211,153,.1);color:var(--green);border:1px solid rgba(52,211,153,.2);}
...
.navbtn.active{background:rgba(52,211,153,.12);color:var(--green);border-color:rgba(52,211,153,.25);}
</style>
...
<div class="wrap">
 <div class="hdr">
  <div class="logo">
   <div class="hex">⚡</div>
   <div><div class="logo-name">OAuth 2.0 + PKCE Demo</div>
    <div class="logo-sub">AUTHORIZATION CODE FLOW -- localhost:5002</div></div>
  </div>
  <div style="display:flex;gap:8px;align-items:center">
   <span class="badge badge-p">OAuth 2.0 + PKCE</span>
   {% if connected %}<span class="badge badge-ok">IDP-Playground Online</span>{% endif %}
  </div>
 </div>

 <div class="card">
  <div class="ch"><div class="ct">Authorization Code Flow with PKCE (RFC 7636)</div></div>
  <div class="flow">
   <div class="step {% if step>=1 %}done{% endif %}">1. Generate PKCE</div><span class="arrow">--></span>
   <div class="step {% if step>=2 %}done{% endif %}">2. /authorize</div><span class="arrow">--></span>
   <div class="step {% if step>=3 %}done{% endif %}">3. User Login</div><span class="arrow">--></span>
   <div class="step {% if step>=4 %}done{% endif %}">4. code callback</div><span class="arrow">--></span>
   <div class="step {% if step>=5 %}done{% endif %}">5. /token exchange</div><span class="arrow">--></span>
   <div class="step {% if step>=6 %}done{% endif %}">6. access_token</div>
  </div>
 </div>
 ...
 <div class="sess">
  ...
  <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
   ...
   <span class="chip chip-p">OAuth2 + PKCE</span>
 ...
 <div class="card" style="max-width:440px;margin:0 auto;text-align:center">
  <div class="cb" style="padding:32px 24px">
   ...
   <div style="font-size:16px;font-weight:700;margin-bottom:8px">OAuth 2.0 Protected Resource</div>
   <div style="font-size:12px;color:var(--muted);margin-bottom:24px;line-height:1.7">
    Click below to start the Authorization Code + PKCE flow.<br>
    A code_verifier and code_challenge (S256) are generated client-side.
   </div>
   <a href="/oauth2/start" class="btn btn-p">Start OAuth2 Flow</a>
   <div style="margin-top:16px;background:rgba(52,211,153,.05);border:1px solid rgba(52,211,153,.15);
    ...
    Client ID: <span style="color:var(--p)">{{ client_id or 'Registering...' }}</span><br>
    Redirect URI: <span style="color:var(--p)">{{ redirect_uri }}</span><br>
    Scopes: <span style="color:var(--p)">{{ scopes }}</span>
   </div>
  </div>
 </div>
 {% endif %}
...
function renderOAUTH(view,d){
  if(d && d.error) return _card('Error','<div style="color:var(--red)">'+_esc(d.error)+'</div>');
  if(view==='overview')
    return _card('OAuth 2.0 + PKCE — Overview',
      _row('Name', d.name||'—')+_row('Email', d.email||'—')+
      _row('Protocol', d.protocol||'OAuth 2.0 + PKCE')+_row('Subject', d.subject)+
      _row('Scope', d.scope)+_row('Group claims', d.group_count));
  if(view==='token')
    return _card('Access Token', _row('Type','Bearer')+_row('Scope', d.scope))+
      _card('Raw Access Token','<div class="tbox" style="word-break:break-all">'+_esc(d.access_token||'')+'</div>')+
      _card('Decoded Claims','<pre class="dec" style="margin:0;white-space:pre-wrap;word-break:break-all">'+_esc(JSON.stringify(d.claims||{},null,2))+'</pre>');
  if(view==='claims'){
    var a=d.claims||{}; var ks=Object.keys(a);
    var rows=ks.length?ks.map(function(k){return _row(k,_fmt(a[k]));}).join(''):'<div style="color:var(--muted);font-size:12px">No claims</div>';
    return _card('Access Token Claims', rows);
  }
  if(view==='groups'){
    var g=d.groups||[];
...
async function nav(view){
  ...
  try{
    ...
    var d=await r.json();
    if(vb) vb.innerHTML=renderOAUTH(view,d);
  }catch(e){ if(vb) vb.innerHTML='<div class="card"><div class="cb" style="color:var(--red)">Error: '+_esc(e.message)+'</div></div>'; }
  window.scrollTo(0,0);
}
window.addEventListener('DOMContentLoaded', function(){
  if(document.getElementById('appnav') && document.getElementById('view-body')){ nav('overview'); }
});
window.addEventListener('DOMContentLoaded', function(){
...
def oauth2_start():
    ...
    return redirect(f"{IDPPLAYGROUND_BASE}/oauth2/authorize?{params}")
...
def oauth2_callback():
    ...
    if error:
        return redirect(url_for("index") + f"?err={error}")
    if state != session.get("oauth_state",""):
        return redirect(url_for("index") + "?err=state_mismatch")
    if not code:
        return redirect(url_for("index") + "?err=no_code")

    session["step"] = 4

    # Exchange code for token using the IDP-Playground token issue API directly
    # (simulates what /oauth2/token would do in a full IDP implementation)
    try:
        app_list = requests.get(f"{IDPPLAYGROUND_BASE}/api/fs/applications", timeout=4).json()
        our_app  = next((a for a in app_list if a["name"] == "OAuth2 Demo Client"), None)
        if not our_app:
            return redirect(url_for("index") + "?err=app_not_found")

        # In a real OAuth2 server the code would be stored and redeemed.
        # IDP-Playground doesn't yet implement the full code grant server-side,
        # so we issue the token directly (same result, demonstrates the flow).
        # The IdP returned its signed access token as the authorization "code".
        # Decode it to recover the real subject (the user's UPN), then exchange
        # that for this app's token so the issued token carries the user's
        # name / email / group claims (not the raw code string).
        code_claims = _decode(code)
        subject = code_claims.get("sub") or code
        tok = requests.post(f"{IDPPLAYGROUND_BASE}/api/fs/tokens/issue", json={
            "app_id":  our_app["id"],
            "subject": subject,
            "lifetime": 3600,
            "scopes":  SCOPES,
        ...
        return redirect(url_for("index") + f"?err={exc}")

    if not tok.get("ok"):
        return redirect(url_for("index") + "?err=token_failed")
...
def api_section(name):
    ...
    if name == "overview":
        return jsonify({"section": "overview", "protocol": "OAuth2 + PKCE",
                        "name": claims.get("name"), "email": claims.get("email"),
                        "subject": claims.get("sub"), "scope": td.get("scope"),
                        "group_count": len(claims.get("groups", []))})
    if name == "token":
        return jsonify({"section": "token", "access_token": td.get("access_token"),
                        "scope": td.get("scope"), "claims": claims})
    if name == "claims":
...
class _Q(WSGIRequestHandler):
    def log_message(self, fmt, *args):
        print(f"  [OAuth2] {fmt % args}", flush=True)
...
if __name__ == "__main__":
    ...
    _register()
    print(f"  IDP-Playground OAuth2 Demo  ->  http://localhost:{PORT}", flush=True)
