# -*- coding: utf-8 -*-
"""
IDP-Playground -- SAML 2.0 Demo Client
File : testclient_saml.py   Port: 5001

SP-initiated SSO flow:
  1. SP builds AuthnRequest (deflate+base64, HTTP-Redirect binding)
  2. User is redirected to IDP-Playground /saml/sso
  3. IDP-Playground POSTs a SAMLResponse to the ACS URL
  4. SP decodes the assertion and shows attributes

Run: python testclient_saml.py
"""

import os, sys, io, json, secrets, base64, zlib, datetime
...
PUBLIC_URL     = os.environ.get("SAML_PUBLIC_URL", "http://localhost:5001")
SP_ENTITY_ID   = PUBLIC_URL + "/saml/metadata"
ACS_URL        = PUBLIC_URL + "/saml/acs"
IDP_SSO_URL    = f"{IDPPLAYGROUND_BASE}/saml/sso"
PORT           = 5001

app = Flask(__name__)
app.secret_key = "idp-playground-saml-client-fixed-session-key-2024"
...
app.config["SESSION_COOKIE_NAME"] = "va_saml_session"
...
@app.route("/version")
def _version():
    return {"build": BUILD_VERSION, "protocol": "SAML"}
...
def _show_error(e):
    ...
    html = ("<html><body style='background:#0b0f1a;color:#e6edf3;font-family:monospace;padding:30px'>"
        "<h1 style='color:#f87171'>SAML client error (" + str(code) + ")</h1>"
...
:root{--bg:#05080d;--bg2:#090e16;--bg3:#0e1620;--b:1px solid #172333;--b2:1px solid #1f3349;
  --p:#a78bfa;--green:#34d399;--red:#f87171;--v:#e8ff47;--text:#d4e4f5;--muted:#4a6685;}
...
.badge-p{background:rgba(167,139,250,.1);border:1px solid rgba(167,139,250,.25);color:var(--p);}
.card{background:var(--bg2);border:1px solid rgba(167,139,250,.15);border-radius:11px;
...
.btn-p{background:var(--p);color:#000;box-shadow:0 0 14px rgba(167,139,250,.3);}
.btn-p:hover{box-shadow:0 0 26px rgba(167,139,250,.5);transform:translateY(-1px);}
...
.mv{text-align:right;word-break:break-all;}
...
pre.xml{font-family:'IBM Plex Mono',monospace;font-size:9px;background:var(--bg3);border:var(--b);
  border-radius:7px;padding:12px;overflow:auto;line-height:1.6;color:var(--muted);max-height:260px;
...
.gtag{display:inline-flex;padding:3px 9px;border-radius:6px;font-size:11px;font-weight:700;
  background:rgba(167,139,250,.1);color:var(--p);border:1px solid rgba(167,139,250,.2);margin:2px;}
...
.av{width:34px;height:34px;border-radius:8px;background:rgba(167,139,250,.2);color:var(--p);
  display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;}
.chip{display:inline-flex;padding:3px 9px;border-radius:20px;font-size:10px;
  font-family:'IBM Plex Mono',monospace;}
.chip-ok{background:rgba(52,211,153,.1);color:var(--green);border:1px solid rgba(52,211,153,.2);}
.chip-p{background:rgba(167,139,250,.1);color:var(--p);border:1px solid rgba(167,139,250,.2);}
...
.navbtn.active{background:rgba(167,139,250,.12);color:var(--p);border-color:rgba(167,139,250,.25);}
</style>
...
<div class="wrap">
 <div class="hdr">
  <div class="logo">
   <div class="hex">📄</div>
   <div><div class="logo-name">SAML 2.0 Demo SP</div>
    <div class="logo-sub">SERVICE PROVIDER -- localhost:5001</div></div>
  </div>
  <div style="display:flex;gap:8px;align-items:center">
   <span class="badge badge-p">SAML 2.0</span>
   {% if connected %}<span class="badge badge-ok">IDP-Playground Online</span>{% endif %}
  </div>
 </div>

 <div class="card">
  <div class="ch"><div class="ct">SP-Initiated SSO Flow</div></div>
  <div class="flow">
   <div class="step {% if step >= 1 %}done{% endif %}">1. Visit SP</div><span class="arrow">--></span>
   <div class="step {% if step >= 2 %}done{% endif %}">2. AuthnRequest</div><span class="arrow">--></span>
   <div class="step {% if step >= 3 %}done{% endif %}">3. IDP Login</div><span class="arrow">--></span>
   <div class="step {% if step >= 4 %}done{% endif %}">4. SAMLResponse POST</div><span class="arrow">--></span>
   <div class="step {% if step >= 5 %}done{% endif %}">5. Assertion Validated</div>
  </div>
 </div>
 ...
 <div class="sess">
  <div style="display:flex;align-items:center;gap:10px">
   <div class="av">{{ assertion.name_id[0]|upper if assertion.name_id else 'U' }}</div>
   <div>
    <div style="font-weight:700;font-size:14px">{{ assertion.attrs.get('name', assertion.name_id) }}</div>
    <div style="font-family:'IBM Plex Mono',monospace;font-size:10px;color:var(--muted)">{{ assertion.name_id }}</div>
   </div>
  </div>
  <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
   <span class="chip chip-ok">SAML Assertion Valid</span>
   <span class="chip chip-p">SP-Initiated SSO</span>
   <a href="/logout" class="btn btn-red" style="padding:5px 12px;font-size:11px">Sign out</a>
  </div>
 </div>

 <!-- ── App navigation menu (demo, generates captured traffic) ── -->
 <div id="appnav" style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:20px;padding:6px;background:var(--bg2);border:var(--b);border-radius:11px">
  ...
  <button class="navbtn" data-view="assertion" onclick="nav('assertion')">📄 Assertion</button>
  <button class="navbtn" data-view="attributes" onclick="nav('attributes')">🏷 Attributes</button>
  <button class="navbtn" data-view="directory" onclick="nav('directory')">👤 Directory</button>
  <button class="navbtn" data-view="metadata" onclick="nav('metadata')">⚙ Metadata</button>
 </div>
 ...
 <div class="card" style="max-width:420px;margin:0 auto;text-align:center">
  <div class="cb" style="padding:32px 24px">
   ...
   <div style="font-size:16px;font-weight:700;margin-bottom:8px">SAML 2.0 Protected Resource</div>
   <div style="font-size:12px;color:var(--muted);margin-bottom:24px;line-height:1.7">
    Click below to start SP-initiated SSO.<br>
    You will be redirected to IDP-Playground to authenticate.
   </div>
   <a href="/saml/login" class="btn btn-p">Initiate SAML SSO</a>
   <div style="margin-top:16px;background:rgba(167,139,250,.05);border:1px solid rgba(167,139,250,.15);
    ...
    SP Entity ID: <span style="color:var(--p)">{{ sp_entity_id }}</span><br>
    ACS URL: <span style="color:var(--p)">{{ acs_url }}</span><br>
    IDP SSO: <span style="color:var(--p)">{{ idp_sso_url }}</span>
   </div>
  </div>
 </div>
 {% endif %}
...
function renderSAML(view,d){
  if(d && d.error) return _card('Error','<div style="color:var(--red)">'+_esc(d.error)+'</div>');
  if(view==='overview')
    return _card('SAML 2.0 SSO — Overview',
      _row('Protocol', d.protocol||'SAML 2.0')+_row('NameID', d.name_id)+
      _row('SSO Flow', d.sso_flow||'SP-initiated')+_row('Attributes received', d.attributes)+
      _row('Assertion', 'Validated ✓'));
  if(view==='assertion')
    return _card('SAML Assertion',
      _row('NameID', d.name_id)+_row('Signature', d.success?'Valid ✓':'Not validated'))+
      _card('Raw SAMLResponse XML','<pre class="xml" style="margin:0;white-space:pre-wrap;word-break:break-all">'+_esc(d.raw_xml||'(not available)')+'</pre>');
  if(view==='attributes'){
    var a=d.attrs||{}; var ks=Object.keys(a);
    var rows=ks.length?ks.map(function(k){return _row(k,_fmt(a[k]));}).join(''):'<div style="color:var(--muted);font-size:12px">No attributes in assertion</div>';
    return _card('SAML Attributes (Claims released to this SP)', rows);
  }
  if(view==='directory'){
  ...
  if(view==='metadata')
    return _card('SP / IDP Metadata',
      _row('SP Entity ID', d.sp_entity_id)+_row('ACS URL', d.acs_url)+_row('IDP SSO URL', d.idp_sso_url));
  return _card(view, '<pre class="xml">'+_esc(JSON.stringify(d,null,2))+'</pre>');
}
async function nav(view){
  ...
  try{
    ...
    var d=await r.json();
    if(vb) vb.innerHTML=renderSAML(view,d);
...
def _register():
    try:
        ...
        if any(a["name"] == "SAML Demo SP" for a in apps):
            return
        requests.post(f"{IDPPLAYGROUND_BASE}/api/fs/applications", json={
            "name": "SAML Demo SP", "protocol": "SAML",
            "redirect_uris": ACS_URL,
            "allowed_scopes": "openid profile email groups",
            "icon_emoji": "📄", "brand_color": "#a78bfa",
        }, timeout=4)
        print("  [IDP-Playground] SAML Demo SP registered", flush=True)
    except Exception as e:
        print(f"  [IDP-Playground] Registration skipped: {e}", flush=True)


def _build_authn_request(req_id: str) -> str:
...
def _parse_saml_response(b64: str) -> dict:
    try:
        ...
        success    = "Success" in (status_el.get("Value","") if status_el is not None else "")
        ...
        return {"success": success, "name_id": name_id,
                "attrs": attrs, "raw_xml": text}
    except Exception as e:
        return {"success": False, "error": str(e), "name_id": "", "attrs": {}, "raw_xml": ""}
...
def saml_login():
    """Build AuthnRequest and redirect to IDP."""
    req_id = "_" + secrets.token_hex(16)
    session["saml_req_id"] = req_id
    session["saml_step"]   = 2
    encoded = _build_authn_request(req_id)
    relay   = base64.b64encode(b"/").decode()
    params  = urlencode({"SAMLRequest": encoded, "RelayState": relay})
    return redirect(f"{IDP_SSO_URL}?{params}")


@app.route("/saml/acs", methods=["POST"])
def saml_acs():
    """Assertion Consumer Service -- receive SAMLResponse from IDP."""
    b64 = request.form.get("SAMLResponse", "")
    if not b64:
        return redirect(url_for("index") + "?err=No+SAMLResponse+received")
    assertion = _parse_saml_response(b64)
    if not assertion.get("success"):
        session["saml_step"] = 1
        return redirect(url_for("index") + "?err=SAML+assertion+not+successful")
...
def api_section(name):
    """
    Backing data for each navigation tab. Fetching a section generates real HTTP
    traffic the capture panel records; some sections call back to IDP-Playground,
    producing captured HTTPS traffic for troubleshooting.
    """
    assertion = session.get("assertion")
    if not assertion:
        return jsonify({"error": "no SAML assertion in session"}), 401

    if name == "overview":
        return jsonify({"section": "overview", "protocol": "SAML 2.0",
                        "name_id": assertion.get("name_id"),
                        "attributes": len(assertion.get("attrs", {})),
                        "sso_flow": "SP-initiated"})
    if name == "assertion":
        return jsonify({"section": "assertion",
                        "name_id": assertion.get("name_id"),
                        "attrs": assertion.get("attrs", {}),
                        "success": assertion.get("success"),
                        "raw_xml": assertion.get("raw_xml", "")})
    if name == "attributes":
        return jsonify({"section": "attributes", "attrs": assertion.get("attrs", {})})
    if name == "directory":
        # Refetch the live user from IDP-Playground -> captured HTTPS call
        live = {}
        try:
            nid = assertion.get("name_id", "")
            r = requests.get(f"{IDPPLAYGROUND_BASE}/api/ds/users", params={"q": nid}, timeout=4)
            rows = r.json()
            live = next((u for u in rows if u.get("upn") == nid or u.get("email") == nid), {})
        except Exception as e:
            live = {"error": str(e)}
        return jsonify({"section": "directory", "live_directory": live})
    if name == "metadata":
        return jsonify({"section": "metadata", "sp_entity_id": SP_ENTITY_ID,
                        "acs_url": ACS_URL, "idp_sso_url": IDP_SSO_URL})
...
class _Q(WSGIRequestHandler):
    def log_message(self, fmt, *args):
        print(f"  [SAML] {fmt % args}", flush=True)
...
if __name__ == "__main__":
    ...
    _register()
    print(f"  IDP-Playground SAML Demo SP  ->  http://localhost:{PORT}", flush=True)
